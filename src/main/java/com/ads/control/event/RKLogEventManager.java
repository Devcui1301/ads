...
public class RKLogEventManager {
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        logEventWithAds(context, (float) adValue.getValueMicros(), adValue.getPrecisionType(), adUnitId, mediationAdapterClassName, RKAd.getInstance().getMediationProvider());
...
//        logEventWithAds(context, (float) adValue.getRevenue(), 0, adValue.getAdUnitId(), adValue.getNetworkName(), ITGAdConfig.PROVIDER_MAX);
    ...
    private static void logEventWithAds(Context context, float revenue, int precision, String adUnitId, String network, int mediationProvider) {
        ...
        SharePreferenceUtils.updateCurrentTotalRevenueAdApplovin(context, (float) revenue);
        ...
        logTotalRevenueAdIn1DaysIfNeed(context);
    }
    ...
    public static void logTotalRevenue001AdApplovin(Context context) {
        float revenue = AppUtil.currentTotalRevenue001Ad;
        if (revenue / 1000000 >= 0.01) {
            AppUtil.currentTotalRevenue001Ad = 0;
            SharePreferenceUtils.updateCurrentTotalRevenue001Ad(context, 0);
    ...
    public static void logTotalRevenueAdIn1DaysIfNeed(Context context) {
        long installTime = SharePreferenceUtils.getInstallTime(context);
        if (!SharePreferenceUtils.isPushRevenue1Day(context)
                && (System.currentTimeMillis() - installTime >= (long) 24 * 60 * 60 * 1000)) {
            Log.d(TAG, "logTotalRevenueAdAt1DaysIfNeed: ");
            logCurrentTotalRevenueAd(context, "event_total_revenue_ad_in_1_days");
            SharePreferenceUtils.setPushedRevenue1Day(context);
        }
    }
